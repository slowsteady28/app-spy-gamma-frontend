...
const PW2AppReady = () => {
  ...
  const refreshPW2Data = () => {
    setLoading(true); // show skeleton immediately

    // Wait 3 seconds, then perform fetch
    setTimeout(() => {
      fetch(`${apiBaseUrl}/api/refresh-pw2`, { method: "POST" })
        .then((res) => {
          if (!res.ok) throw new Error("Failed to refresh PW2 data");
          return res.json();
        })
        .then(() => setLastUpdated(new Date()))
        .catch((err) => console.error("Refresh PW2 error:", err))
