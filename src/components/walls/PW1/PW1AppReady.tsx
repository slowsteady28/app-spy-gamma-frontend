...
const PW1AppReady = () => {
  ...
  const refreshPW1Data = () => {
    setLoading(true); // show skeleton immediately

    // Wait 3 seconds, then perform fetch
    setTimeout(() => {
      fetch(`${apiBaseUrl}/api/refresh-pw1`, { method: "POST" })
        .then((res) => {
          if (!res.ok) throw new Error("Failed to refresh PW1 data");
          return res.json();
        })
        .then(() => setLastUpdated(new Date()))
        .catch((err) => console.error("Refresh PW1 error:", err))
