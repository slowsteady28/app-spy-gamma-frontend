...
const AGS2HisGammaChart = ({ lookback }: Props) => {
    ...
    const fetchData = async () => {
      ...
      try {
        const response = await axios.get(
          `${apiBaseUrl}/data/ags2-history?lookback=${lookback}`
        );
        ...
        const traceAGS2 = {
          ...
          hovertemplate: "Date: %{x}<br>AGS2: %{y:.0f}<extra></extra>",
        };
        ...
        setData([traceAGS2, traceCandle]);
      } catch (err: any) {
        setError(`Failed to load AGS2 data: ${err.message}`);
      } finally {
        setIsLoading(false);
      }
    };
  ...
  useEffect(() => {
    ...
    if (el && index !== -1) {
      try {
        ...
        console.error("Fx.hover failed in AGS2HisGammaChart", err);
      }
    }
  }, [hoveredDate, data]);
  ...
  if (isLoading && data.length === 0) {
    return (
      <div className="d-flex justify-content-center align-items-center p-5">
        ...
        <span className="ms-3">Loading AGS2 data...</span>
      </div>
    );
  }
...
export default AGS2HisGammaChart;
