...
const AGS2CallPutGammaChart = ({ lookback }: Props) => {
  ...
  useEffect(() => {
    axios
      ...
      .catch((err) =>
        console.error("Error loading AGS2 Call/Put Gamma data", err)
      );
  }, [lookback]);
  ...
  useEffect(() => {
    ...
    if (el && index !== -1) {
      try {
        ...
        console.error("Fx.hover failed in AGS2CallPutGammaChart", err);
      }
    }
...
export default AGS2CallPutGammaChart;
