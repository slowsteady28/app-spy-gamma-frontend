...
function AbsGamma2CallPutGammaChart({ lookback }: Props) {
  const [data, setData] = useState<GammaDataPoint[]>([]);
  const [visible, setVisible] = useState<"both" | "call" | "put">("both");

  useEffect(() => {
    axios
      .get(`${apiBaseUrl}/data/ags2-gamma?lookback=${lookback}`)
      .then((res) => {
        console.log("AbsGamma2 Call/Put Gamma Data:", res.data);
        setData(res.data);
      })
      .catch((err) => console.error("Error loading AbsGamma2 gamma data", err));
  }, [lookback]);
...
export default AbsGamma2CallPutGammaChart;
