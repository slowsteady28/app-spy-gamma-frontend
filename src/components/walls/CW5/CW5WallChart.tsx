  ...
  const fetchData = async () => {
    ...
    try {
      const response = await axios.get(
        `${apiBaseUrl}/data/cw5-history?lookback=${lookback}`
      );
      const formattedData = response.data.map((item: any) => {
        return {
          date: item["date"] || item["Date"],
          price: Number(item["price"] || item["Price"]),
          cw5: Number(item["cw5"] || item["CW5"]),
        ...
        data={[
          {
            ...
            y: wallPriceData.map((item) => item.cw5),
            name: "CW",
            mode: "lines+markers",
            line: {
              color: "0096b4",
            ...
            ? [
                {
                  ...
                  mode: "lines+markers" as const,
                  x: wallPriceData.map((item) => item.date),
                  y: wallPriceData.map((item) => item.pw5),
