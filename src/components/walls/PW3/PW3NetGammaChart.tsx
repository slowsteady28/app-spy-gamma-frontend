...
function PW3NetGammaChart({ lookback }: PutNetGammaChartProps) {
  const [data, setData] = useState<NetGammaDataPoint[]>([]);

  useEffect(() => {
    axios
      .get(`${apiBaseUrl}/data/pw3-net-gamma?lookback=${lookback}`)
      .then((res) => {
        console.log("PW3 Net Gamma Data:", res.data);
        setData(res.data);
      })
      .catch((err) => console.error("Error loading PW3 net gamma data", err));
  }, [lookback]);
...
export default PW3NetGammaChart;
