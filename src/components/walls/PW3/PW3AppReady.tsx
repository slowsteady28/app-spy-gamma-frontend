...
import SPYHourlyChart from "../../charts/PW3SPYHourlyChart";
...
const PW3AppReady = () => {
  ...
  const refreshPW3Data = () => {
    setLoading(true); // show skeleton immediately

    // Wait 3 seconds, then perform fetch
    setTimeout(() => {
      fetch(`${apiBaseUrl}/api/refresh-pw3`, { method: "POST" })
        .then((res) => {
          if (!res.ok) throw new Error("Failed to refresh PW3 data");
          return res.json();
        })
        .then(() => setLastUpdated(new Date()))
        .catch((err) => console.error("Refresh PW3 error:", err))
