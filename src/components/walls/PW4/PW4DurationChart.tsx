...
function PW4DurationChart({ lookback }: PutDurationChartProps) {
  const [data, setData] = useState<DurationDataPoint[]>([]);

  useEffect(() => {
    axios
      .get(`${apiBaseUrl}/data/pw4-duration?lookback=${lookback}`)
      .then((res) => {
        console.log("Put Duration Data:", res.data);
        setData(res.data);
      })
      .catch((err) => console.error("Error loading PW4 duration data", err));
  }, [lookback]);
...
export default PW4DurationChart;
