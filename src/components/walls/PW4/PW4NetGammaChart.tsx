...
function PW4NetGammaChart({ lookback }: PutNetGammaChartProps) {
  const [data, setData] = useState<NetGammaDataPoint[]>([]);

  useEffect(() => {
    axios
      .get(`${apiBaseUrl}/data/pw4-net-gamma?lookback=${lookback}`)
      .then((res) => {
        console.log("PW4 Net Gamma Data:", res.data);
        setData(res.data);
      })
      .catch((err) => console.error("Error loading PW4 net gamma data", err));
  }, [lookback]);
...
export default PW4NetGammaChart;
