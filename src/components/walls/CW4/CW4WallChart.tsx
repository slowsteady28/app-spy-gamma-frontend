  ...
  const fetchData = async () => {
    ...
    try {
      const response = await axios.get(
        `${apiBaseUrl}/data/cw4-history?lookback=${lookback}`
      );
      const formattedData = response.data.map((item: any) => {
        return {
          date: item["date"] || item["Date"],
          price: Number(item["price"] || item["Price"]),
          cw4: Number(item["cw4"] || item["CW4"]),
  ...
  if (isLoading && wallPriceData.length === 0) {
    return (
      <div className="d-flex justify-content-center align-items-center p-5">
        ...
        <span className="ms-3">Loading call wall data...</span>
      </div>
    );
  }
  ...
  if (wallPriceData.length === 0) {
    return (
      <div className="alert alert-warning m-3" role="alert">
        ...
        <p>Unable to retrieve call wall data at this time.</p>
        <button className="btn btn-outline-primary" onClick={fetchData}>
          Try Again
        </button>
      </div>
        ...
        data={[
          {
            ...
            y: wallPriceData.map((item) => item.cw4),
            name: "CW",
            mode: "lines+markers",
            line: {
              color: "0096b4",
            ...
            ? [
                {
                  ...
                  mode: "lines+markers" as const,
                  x: wallPriceData.map((item) => item.date),
                  y: wallPriceData.map((item) => item.pw4),
