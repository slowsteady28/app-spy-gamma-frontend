...
import SPYHourlyChart from "../../charts/CW4SPYHourlyChart";
...
const CW4AppReady = () => {
  ...
  const refreshCW4Data = () => {
    setLoading(true); // show skeleton immediately

    // Wait 3 seconds, then perform fetch
    setTimeout(() => {
      fetch(`${apiBaseUrl}/api/refresh-cw4`, { method: "POST" })
        .then((res) => {
          if (!res.ok) throw new Error("Failed to refresh CW4 data");
          return res.json();
        })
        .then(() => setLastUpdated(new Date()))
        .catch((err) => console.error("Refresh CW4 error:", err))
