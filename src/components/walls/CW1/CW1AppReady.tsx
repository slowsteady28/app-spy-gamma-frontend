...
const CW1AppReady = () => {
  ...
  const refreshCW1Data = () => {
    setLoading(true); // show skeleton immediately

    // Wait 3 seconds, then perform fetch
    setTimeout(() => {
      fetch(`${apiBaseUrl}/api/refresh-cw1`, { method: "POST" })
        .then((res) => {
          if (!res.ok) throw new Error("Failed to refresh CW1 data");
          return res.json();
        })
        .then(() => setLastUpdated(new Date()))
        .catch((err) => console.error("Refresh CW1 error:", err))
