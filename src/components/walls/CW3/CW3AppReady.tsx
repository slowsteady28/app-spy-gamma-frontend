...
import CW3HoverSyncChart from "./CW3HoverSyncChart";
...
const CW3AppReady = () => {
  ...
  const refreshCW3Data = () => {
    setLoading(true); // show skeleton immediately

    // Wait 3 seconds, then perform fetch
    setTimeout(() => {
      fetch(`${apiBaseUrl}/api/refresh-cw3`, { method: "POST" })
        .then((res) => {
          if (!res.ok) throw new Error("Failed to refresh CW3 data");
          return res.json();
        })
        .then(() => setLastUpdated(new Date()))
        .catch((err) => console.error("Refresh CW3 error:", err))
