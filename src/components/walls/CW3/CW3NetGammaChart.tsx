...
function CW3NetGammaChart({ lookback }: PutNetGammaChartProps) {
  const [data, setData] = useState<NetGammaDataPoint[]>([]);

  useEffect(() => {
    axios
      .get(`${apiBaseUrl}/data/cw3-net-gamma?lookback=${lookback}`)
      .then((res) => {
        console.log("cW3 Net Gamma Data:", res.data);
        setData(res.data);
      })
      .catch((err) => console.error("Error loading cW3 net gamma data", err));
  }, [lookback]);
...
export default CW3NetGammaChart;
