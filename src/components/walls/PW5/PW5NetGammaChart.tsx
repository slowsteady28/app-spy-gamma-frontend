...
function PW5NetGammaChart({ lookback }: PutNetGammaChartProps) {
  const [data, setData] = useState<NetGammaDataPoint[]>([]);

  useEffect(() => {
    axios
      .get(`${apiBaseUrl}/data/pw5-net-gamma?lookback=${lookback}`)
      .then((res) => {
        console.log("PW5 Net Gamma Data:", res.data);
        setData(res.data);
      })
      .catch((err) => console.error("Error loading PW5 net gamma data", err));
  }, [lookback]);
...
export default PW5NetGammaChart;
