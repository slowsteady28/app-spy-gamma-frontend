...
import PW4WallChart from "./PW5WallChart";
import PW4NetOIChart from "./PW5NetOIChart";
...
const PW4AppReady = () => {
  ...
  const refreshPW4Data = () => {
    setLoading(true); // show skeleton immediately

    // Wait 3 seconds, then perform fetch
    setTimeout(() => {
      fetch(`${apiBaseUrl}/api/refresh-pw4`, { method: "POST" })
        .then((res) => {
          if (!res.ok) throw new Error("Failed to refresh PW4 data");
          return res.json();
        })
        .then(() => setLastUpdated(new Date()))
        .catch((err) => console.error("Refresh PW4 error:", err))
