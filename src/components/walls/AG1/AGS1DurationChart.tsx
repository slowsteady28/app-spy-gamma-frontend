...
const AGS1DurationChart = ({ lookback }: Props) => {
  ...
  useEffect(() => {
    axios
      .get(`${apiBaseUrl}/data/ags1-duration?lookback=${lookback}`)
      .then((res) => {
        const arr = Array.isArray(res.data) ? res.data : res.data.data;
        setData(arr);
      })
      .catch((err) => console.error("Error loading AGS1 Duration data", err));
  }, [lookback]);
  ...
  useEffect(() => {
    ...
    if (el && index !== -1) {
      try {
        ...
        console.error("Fx.hover failed in AGS1DurationChart", err);
      }
    }
  }, [hoveredDate, data]);
...
export default AGS1DurationChart;
