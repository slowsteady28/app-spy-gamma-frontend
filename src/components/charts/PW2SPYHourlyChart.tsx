...
interface CandleData {
  ...
  PW2?: number;
  "Percentile Z-Score CW Gamma (Net) PW2"?: number;
  durationD?: number; // Percentile Z-Score Ave Length PW2 (0..1)
}

// Colors
const PW2_DEFAULT = "#6F42C1";
const PW2_NEON_GREEN = "#39FF14";
const PW2_REDDISH = "#FFCDD2";
...
const SPYHourlyChart: React.FC<SPYHourlyChartProps> = ({ lookback }) => {
  ...
  const colorForPercentile = (p?: number) => {
    if (p == null || Number.isNaN(p)) return PW2_DEFAULT;
    if (p >= percentileHigh) return PW2_NEON_GREEN;
    if (p <= percentileLow) return PW2_REDDISH;
    return PW2_DEFAULT;
  };
      ...
      .then((res) => {
        ...
        const categorized = rawData.map((d) => {
          ...
          const percentile = parseFloat(
            (d as any)["Percentile Z-Score CW Gamma (Net) PW2"] as any
          );
          const durationD = parseFloat(
            (d as any)["Percentile Z-Score Ave Length PW2"] as any
          );
          ...
          return {
            ...d,
            category,
            "Percentile Z-Score CW Gamma (Net) PW2": percentile,
            durationD,
          };
        });
  ...
  // Unique PW2 per date + percentile
  const Pw2ShapesMap: { [date: string]: { Pw2: number; p?: number } } = {};
  data.forEach((d) => {
    if (typeof d.PW2 === "number" && !(d.Date in Pw2ShapesMap)) {
      Pw2ShapesMap[d.Date] = {
        Pw2: d.PW2,
        p: d["Percentile Z-Score CW Gamma (Net) PW2"],
      };
    }
  });
  ...
  // PW2 daily horizontal line shapes (color reacts to sliders)
  const Pw2DailyShapes: Partial<Shape>[] = Object.entries(Pw2ShapesMap)
  ...
  const shapes: Partial<Shape>[] = [
    ...(showCallWall ? Pw2DailyShapes : []),
    ...dayRingsLowD,
    ...dayRingsHighD,
  ];

  // PW2 line data for colored bands + connector (bands react to sliders)
  const Pw2LineData = data
    .filter((d) => typeof d.PW2 === "number")
    .map((d) => ({
      x: `${d.Date} ${d.Time}`,
      y: d.PW2 as number,
      p: d["Percentile Z-Score CW Gamma (Net) PW2"],
  ...
  const Pw2Connector: Partial<PlotData> = {
    type: "scatter",
    mode: "lines",
    name: "PW2 connector",
    x: Pw2LineData.map((d) => d.x),
    y: Pw2LineData.map((d) => d.y),
    line: { color: PW2_DEFAULT, width: 1 },
  ...
  const makePw2Trace = (
    ...
    mode: "lines+markers",
    name: label,
    x: Pw2LineData.map((d) => (predicate(d.p) ? d.x : null)) as any,
    y: Pw2LineData.map((d) => (predicate(d.p) ? d.y : null)) as any,
  ...
  const Pw2High = makePw2Trace(
    `PW2 ≥ ${(percentileHigh * 100).toFixed(0)}th pct`,
    PW2_NEON_GREEN,
    (p) => (p ?? -1) >= percentileHigh
  );
  const Pw2Low = makePw2Trace(
    `PW2 ≤ ${(percentileLow * 100).toFixed(0)}th pct`,
    PW2_REDDISH,
    (p) => (p ?? 1) <= percentileLow
  );
  const Pw2Mid = makePw2Trace(
    `PW2 mid`,
    PW2_DEFAULT,
  ...
  if (showCallWall) {
    plotSeries.push(Pw2Connector, Pw2High, Pw2Mid, Pw2Low);
  }
