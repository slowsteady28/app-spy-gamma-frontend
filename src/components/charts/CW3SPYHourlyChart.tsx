...
interface CandleData {
  ...
  CW3?: number;
  "Percentile Z-Score CW Gamma (Net) CW3"?: number;
  durationD?: number; // Percentile Z-Score Ave Length CW3 (0..1)
}

// Colors
const CW3_DEFAULT = "#33C3F0";
const CW3_NEON_GREEN = "#39FF14";
const CW3_REDDISH = "#FFCDD2";
...
const SPYHourlyChart: React.FC<SPYHourlyChartProps> = ({ lookback }) => {
  ...
  const colorForPercentile = (p?: number) => {
    if (p == null || Number.isNaN(p)) return CW3_DEFAULT;
    if (p >= percentileHigh) return CW3_NEON_GREEN;
    if (p <= percentileLow) return CW3_REDDISH;
    return CW3_DEFAULT;
  };
      ...
      .then((res) => {
        ...
        const categorized = rawData.map((d) => {
          ...
          const percentile = parseFloat(
            (d as any)["Percentile Z-Score CW Gamma (Net) CW3"] as any
          );
          const durationD = parseFloat(
            (d as any)["Percentile Z-Score Ave Length CW3"] as any
          );
          ...
          return {
            ...d,
            category,
            "Percentile Z-Score CW Gamma (Net) CW3": percentile,
            durationD,
          };
        });
  ...
  // Unique CW3 per date + percentile
  const cw3ShapesMap: { [date: string]: { cw3: number; p?: number } } = {};
  data.forEach((d) => {
    if (typeof d.CW3 === "number" && !(d.Date in cw3ShapesMap)) {
      cw3ShapesMap[d.Date] = {
        cw3: d.CW3,
        p: d["Percentile Z-Score CW Gamma (Net) CW3"],
      };
    }
  });
  ...
  // CW3 daily horizontal line shapes (color reacts to sliders)
  const cw3DailyShapes: Partial<Shape>[] = Object.entries(cw3ShapesMap)
  ...
  const shapes: Partial<Shape>[] = [
    ...(showCallWall ? cw3DailyShapes : []),
    ...dayRingsLowD,
    ...dayRingsHighD,
  ];

  // CW3 line data for colored bands + connector (bands react to sliders)
  const cw3LineData = data
    .filter((d) => typeof d.CW3 === "number")
    .map((d) => ({
      x: `${d.Date} ${d.Time}`,
      y: d.CW3 as number,
      p: d["Percentile Z-Score CW Gamma (Net) CW3"],
  ...
  const cw3Connector: Partial<PlotData> = {
    type: "scatter",
    mode: "lines",
    name: "CW3 connector",
    x: cw3LineData.map((d) => d.x),
    y: cw3LineData.map((d) => d.y),
    line: { color: CW3_DEFAULT, width: 1 },
  ...
  const makeCw3Trace = (
    ...
    mode: "lines+markers",
    name: label,
    x: cw3LineData.map((d) => (predicate(d.p) ? d.x : null)) as any,
    y: cw3LineData.map((d) => (predicate(d.p) ? d.y : null)) as any,
  ...
  const cw3High = makeCw3Trace(
    `CW3 ≥ ${(percentileHigh * 100).toFixed(0)}th pct`,
    CW3_NEON_GREEN,
    (p) => (p ?? -1) >= percentileHigh
  );
  const cw3Low = makeCw3Trace(
    `CW3 ≤ ${(percentileLow * 100).toFixed(0)}th pct`,
    CW3_REDDISH,
    (p) => (p ?? 1) <= percentileLow
  );
  const cw3Mid = makeCw3Trace(
    `CW3 mid`,
    CW3_DEFAULT,
  ...
  if (showCallWall) {
    plotSeries.push(cw3Connector, cw3High, cw3Mid, cw3Low);
  }
