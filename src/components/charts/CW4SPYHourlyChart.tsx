...
interface CandleData {
  ...
  CW4?: number;
  "Percentile Z-Score CW Gamma (Net) CW4"?: number;
  durationD?: number; // Percentile Z-Score Ave Length CW4 (0..1)
}

// Colors
const CW4_DEFAULT = "#33C3F0";
const CW4_NEON_GREEN = "#39FF14";
const CW4_REDDISH = "#FFCDD2";
...
const SPYHourlyChart: React.FC<SPYHourlyChartProps> = ({ lookback }) => {
  ...
  const colorForPercentile = (p?: number) => {
    if (p == null || Number.isNaN(p)) return CW4_DEFAULT;
    if (p >= percentileHigh) return CW4_NEON_GREEN;
    if (p <= percentileLow) return CW4_REDDISH;
    return CW4_DEFAULT;
  };
      ...
      .then((res) => {
        ...
        const categorized = rawData.map((d) => {
          ...
          const percentile = parseFloat(
            (d as any)["Percentile Z-Score CW Gamma (Net) CW4"] as any
          );
          const durationD = parseFloat(
            (d as any)["Percentile Z-Score Ave Length CW4"] as any
          );
          ...
          return {
            ...d,
            category,
            "Percentile Z-Score CW Gamma (Net) CW4": percentile,
            durationD,
          };
        });
  ...
  // Unique CW4 per date + percentile
  const cw4ShapesMap: { [date: string]: { cw4: number; p?: number } } = {};
  data.forEach((d) => {
    if (typeof d.CW4 === "number" && !(d.Date in cw4ShapesMap)) {
      cw4ShapesMap[d.Date] = {
        cw4: d.CW4,
        p: d["Percentile Z-Score CW Gamma (Net) CW4"],
      };
    }
  });
  ...
  // CW4 daily horizontal line shapes (color reacts to sliders)
  const cw4DailyShapes: Partial<Shape>[] = Object.entries(cw4ShapesMap)
  ...
  const shapes: Partial<Shape>[] = [
    ...(showCallWall ? cw4DailyShapes : []),
    ...dayRingsLowD,
    ...dayRingsHighD,
  ];

  // CW4 line data for colored bands + connector (bands react to sliders)
  const cw4LineData = data
    .filter((d) => typeof d.CW4 === "number")
    .map((d) => ({
      x: `${d.Date} ${d.Time}`,
      y: d.CW4 as number,
      p: d["Percentile Z-Score CW Gamma (Net) CW4"],
  ...
  const cw4Connector: Partial<PlotData> = {
    type: "scatter",
    mode: "lines",
    name: "CW4 connector",
    x: cw4LineData.map((d) => d.x),
    y: cw4LineData.map((d) => d.y),
    line: { color: CW4_DEFAULT, width: 1 },
  ...
  const makeCw4Trace = (
    ...
    mode: "lines+markers",
    name: label,
    x: cw4LineData.map((d) => (predicate(d.p) ? d.x : null)) as any,
    y: cw4LineData.map((d) => (predicate(d.p) ? d.y : null)) as any,
  ...
  const cw4High = makeCw4Trace(
    `CW4 ≥ ${(percentileHigh * 100).toFixed(0)}th pct`,
    CW4_NEON_GREEN,
    (p) => (p ?? -1) >= percentileHigh
  );
  const cw4Low = makeCw4Trace(
    `CW4 ≤ ${(percentileLow * 100).toFixed(0)}th pct`,
    CW4_REDDISH,
    (p) => (p ?? 1) <= percentileLow
  );
  const cw4Mid = makeCw4Trace(
    `CW4 mid`,
    CW4_DEFAULT,
  ...
  if (showCallWall) {
    plotSeries.push(cw4Connector, cw4High, cw4Mid, cw4Low);
  }
