...
// Colors
const PW3_DEFAULT = "#6F42C1";
const PW3_NEON_GREEN = "#39FF14";
const PW3_REDDISH = "#FFCDD2";
...
const SPYHourlyChart: React.FC<SPYHourlyChartProps> = ({ lookback }) => {
  ...
  const colorForPercentile = (p?: number) => {
    if (p == null || Number.isNaN(p)) return PW3_DEFAULT;
    if (p >= percentileHigh) return PW3_NEON_GREEN;
    if (p <= percentileLow) return PW3_REDDISH;
    return PW3_DEFAULT;
  };
  ...
  // Unique PW3 per date + percentile
  const Pw3ShapesMap: { [date: string]: { Pw3: number; p?: number } } = {};
  data.forEach((d) => {
  ...
  // PW3 daily horizontal line shapes (color reacts to sliders)
  const Pw3DailyShapes: Partial<Shape>[] = Object.entries(Pw3ShapesMap)
  ...
  const shapes: Partial<Shape>[] = [
    ...(showCallWall ? Pw3DailyShapes : []),
    ...dayRingsLowD,
    ...dayRingsHighD,
  ];

  // PW3 line data for colored bands + connector (bands react to sliders)
  const Pw3LineData = data
    .filter((d) => typeof d.PW3 === "number")
    .map((d) => ({
      x: `${d.Date} ${d.Time}`,
      y: d.PW3 as number,
      p: d["Percentile Z-Score CW Gamma (Net) PW3"],
  ...
  const Pw3Connector: Partial<PlotData> = {
    ...
    mode: "lines",
    name: "PW3 connector",
    x: Pw3LineData.map((d) => d.x),
    y: Pw3LineData.map((d) => d.y),
    line: { color: PW3_DEFAULT, width: 1 },
  ...
  const makePw3Trace = (
    ...
    mode: "lines+markers",
    name: label,
    x: Pw3LineData.map((d) => (predicate(d.p) ? d.x : null)) as any,
    y: Pw3LineData.map((d) => (predicate(d.p) ? d.y : null)) as any,
  ...
  const Pw3High = makePw3Trace(
    `PW3 ≥ ${(percentileHigh * 100).toFixed(0)}th pct`,
    PW3_NEON_GREEN,
    (p) => (p ?? -1) >= percentileHigh
  );
  const Pw3Low = makePw3Trace(
    `PW3 ≤ ${(percentileLow * 100).toFixed(0)}th pct`,
    PW3_REDDISH,
    (p) => (p ?? 1) <= percentileLow
  );
  const Pw3Mid = makePw3Trace(
    `PW3 mid`,
    PW3_DEFAULT,
  ...
  if (showCallWall) {
    plotSeries.push(Pw3Connector, Pw3High, Pw3Mid, Pw3Low);
  }
