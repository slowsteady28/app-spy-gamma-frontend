...
interface CandleData {
  ...
  CW2?: number;
  "Percentile Z-Score CW Gamma (Net) CW2"?: number;
  durationD?: number; // Percentile Z-Score Ave Length CW2 (0..1)
}

// Colors
const CW2_DEFAULT = "#33C3F0";
const CW2_NEON_GREEN = "#39FF14";
const CW2_REDDISH = "#FFCDD2";
...
const SPYHourlyChart: React.FC<SPYHourlyChartProps> = ({ lookback }) => {
  ...
  const colorForPercentile = (p?: number) => {
    if (p == null || Number.isNaN(p)) return CW2_DEFAULT;
    if (p >= percentileHigh) return CW2_NEON_GREEN;
    if (p <= percentileLow) return CW2_REDDISH;
    return CW2_DEFAULT;
  };
      ...
      .then((res) => {
        ...
        const categorized = rawData.map((d) => {
          ...
          const percentile = parseFloat(
            (d as any)["Percentile Z-Score CW Gamma (Net) CW2"] as any
          );
          const durationD = parseFloat(
            (d as any)["Percentile Z-Score Ave Length CW2"] as any
          );
          ...
          return {
            ...d,
            category,
            "Percentile Z-Score CW Gamma (Net) CW2": percentile,
            durationD,
          };
        });
  ...
  // Unique CW2 per date + percentile
  const cw2ShapesMap: { [date: string]: { cw2: number; p?: number } } = {};
  data.forEach((d) => {
    if (typeof d.CW2 === "number" && !(d.Date in cw2ShapesMap)) {
      cw2ShapesMap[d.Date] = {
        cw2: d.CW2,
        p: d["Percentile Z-Score CW Gamma (Net) CW2"],
      };
    }
  });
  ...
  // CW2 daily horizontal line shapes (color reacts to sliders)
  const cw2DailyShapes: Partial<Shape>[] = Object.entries(cw2ShapesMap)
  ...
  const shapes: Partial<Shape>[] = [
    ...(showCallWall ? cw2DailyShapes : []),
    ...dayRingsLowD,
    ...dayRingsHighD,
  ];

  // CW2 line data for colored bands + connector (bands react to sliders)
  const cw2LineData = data
    .filter((d) => typeof d.CW2 === "number")
    .map((d) => ({
      x: `${d.Date} ${d.Time}`,
      y: d.CW2 as number,
      p: d["Percentile Z-Score CW Gamma (Net) CW2"],
  ...
  const cw2Connector: Partial<PlotData> = {
    type: "scatter",
    mode: "lines",
    name: "CW2 connector",
    x: cw2LineData.map((d) => d.x),
    y: cw2LineData.map((d) => d.y),
    line: { color: CW2_DEFAULT, width: 1 },
  ...
  const makeCw2Trace = (
    ...
    mode: "lines+markers",
    name: label,
    x: cw2LineData.map((d) => (predicate(d.p) ? d.x : null)) as any,
    y: cw2LineData.map((d) => (predicate(d.p) ? d.y : null)) as any,
  ...
  const cw2High = makeCw2Trace(
    `CW2 ≥ ${(percentileHigh * 100).toFixed(0)}th pct`,
    CW2_NEON_GREEN,
    (p) => (p ?? -1) >= percentileHigh
  );
  const cw2Low = makeCw2Trace(
    `CW2 ≤ ${(percentileLow * 100).toFixed(0)}th pct`,
    CW2_REDDISH,
    (p) => (p ?? 1) <= percentileLow
  );
  const cw2Mid = makeCw2Trace(
    `CW2 mid`,
    CW2_DEFAULT,
  ...
  if (showCallWall) {
    plotSeries.push(cw2Connector, cw2High, cw2Mid, cw2Low);
  }
