...
interface CandleData {
  ...
  PW4?: number;
  "Percentile Z-Score PW Gamma (Net) PW4"?: number;
  durationD?: number; // Percentile Z-Score Ave Length PW4 (0..1)
}

// Colors
const PW4_DEFAULT = "#6F42C1";
const PW4_NEON_GREEN = "#39FF14";
const PW4_REDDISH = "#FFCDD2";
...
const SPYHourlyChart: React.FC<SPYHourlyChartProps> = ({ lookback }) => {
  ...
  const colorForPercentile = (p?: number) => {
    if (p == null || Number.isNaN(p)) return PW4_DEFAULT;
    if (p >= percentileHigh) return PW4_NEON_GREEN;
    if (p <= percentileLow) return PW4_REDDISH;
    return PW4_DEFAULT;
  };
      ...
      .then((res) => {
        ...
        const categorized = rawData.map((d) => {
          ...
          const percentile = parseFloat(
            (d as any)["Percentile Z-Score PW Gamma (Net) PW4"] as any
          );
          const durationD = parseFloat(
            (d as any)["Percentile Z-Score Ave Length PW4"] as any
          );
          ...
          return {
            ...d,
            category,
            "Percentile Z-Score PW Gamma (Net) PW4": percentile,
            durationD,
          };
        });
  ...
  // Unique PW4 per date + percentile
  const Pw4ShapesMap: { [date: string]: { Pw4: number; p?: number } } = {};
  data.forEach((d) => {
    if (typeof d.PW4 === "number" && !(d.Date in Pw4ShapesMap)) {
      Pw4ShapesMap[d.Date] = {
        Pw4: d.PW4,
        p: d["Percentile Z-Score PW Gamma (Net) PW4"],
      };
    }
  });
  ...
  // PW4 daily horizontal line shapes (color reacts to sliders)
  const Pw4DailyShapes: Partial<Shape>[] = Object.entries(Pw4ShapesMap)
  ...
  const shapes: Partial<Shape>[] = [
    ...(showCallWall ? Pw4DailyShapes : []),
    ...dayRingsLowD,
    ...dayRingsHighD,
  ];

  // PW4 line data for colored bands + connector (bands react to sliders)
  const Pw4LineData = data
    .filter((d) => typeof d.PW4 === "number")
    .map((d) => ({
      x: `${d.Date} ${d.Time}`,
      y: d.PW4 as number,
      p: d["Percentile Z-Score PW Gamma (Net) PW4"],
  ...
  const Pw4Connector: Partial<PlotData> = {
    type: "scatter",
    mode: "lines",
    name: "PW4 connector",
    x: Pw4LineData.map((d) => d.x),
    y: Pw4LineData.map((d) => d.y),
    line: { color: PW4_DEFAULT, width: 1 },
  ...
  const makePw4Trace = (
    ...
    mode: "lines+markers",
    name: label,
    x: Pw4LineData.map((d) => (predicate(d.p) ? d.x : null)) as any,
    y: Pw4LineData.map((d) => (predicate(d.p) ? d.y : null)) as any,
  ...
  const Pw4High = makePw4Trace(
    `PW4 ≥ ${(percentileHigh * 100).toFixed(0)}th pct`,
    PW4_NEON_GREEN,
    (p) => (p ?? -1) >= percentileHigh
  );
  const Pw4Low = makePw4Trace(
    `PW4 ≤ ${(percentileLow * 100).toFixed(0)}th pct`,
    PW4_REDDISH,
    (p) => (p ?? 1) <= percentileLow
  );
  const Pw4Mid = makePw4Trace(
    `PW4 mid`,
    PW4_DEFAULT,
  ...
  if (showCallWall) {
    plotSeries.push(Pw4Connector, Pw4High, Pw4Mid, Pw4Low);
  }
